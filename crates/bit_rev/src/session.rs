...
use crate::rate::{AltSchedule, BandwidthLimiters, RateLimiter, SessionLimits, DEFAULT_BURST};
...
/// Per-torrent lifecycle. `Queued` waits for a download/upload slot (issue 38).
...
pub enum ControlError {
    ...
    #[error("failed to move torrent {id}: {message}")]
    MoveFailed { id: TorrentId, message: String },
}

const EVENT_CHANNEL_CAPACITY: usize = 256;
const RATE_TICK: Duration = Duration::from_secs(1);
const RATE_EMA_ALPHA: f64 = 0.2;
const SLOW_RATE_BPS: u64 = 1024;
pub const DEFAULT_MAX_ACTIVE_DOWNLOADS: usize = 5;
pub const DEFAULT_MAX_ACTIVE_UPLOADS: usize = 8;
pub const DEFAULT_QUEUE_SLOW_WINDOW: Duration = Duration::from_secs(30);
...
pub struct SessionOptions {
    ...
    /// Default data directory when add does not pass `save_path`.
    pub download_dir: PathBuf,
    /// Move completed torrents here when `auto_tmm` is on. Empty disables it.
    pub completed_dir: PathBuf,
    /// Poll this directory for `.torrent` / magnet files. Empty disables it.
    pub watch_dir: PathBuf,
    /// Delete watched files after add instead of moving them to `watch-processed`.
...
pub struct Session {
    ...
    categories: Arc<Mutex<BTreeMap<String, Category>>>,
...
impl AddTorrentOptions {
    ...
    pub fn auto_tmm(mut self, enabled: bool) -> Self {
        self.auto_tmm = Some(enabled);
        self
    }

    pub fn force_start(mut self, enabled: bool) -> Self {
        self.force_start = enabled;
        self
    }

    pub fn ratio_limit(mut self, limit: f64) -> Self {
        self.ratio_limit = Some(limit);
        self
    }

    pub fn seeding_time_limit(mut self, minutes: u64) -> Self {
        self.seeding_time_limit = Some(minutes);
        self
    }
...
impl Session {
    ...
    fn with_connector_and_utp(
        ...
        let limits = Arc::new(SessionLimits::new(
            ...
            options.alt_download_limit,
            options.alt_upload_limit,
        ));
        let session = Self {
            ...
            event_tx,
            categories: Arc::new(Mutex::new(BTreeMap::new())),
            limits,
    ...
    async fn restore_resume(
        ...
        let mut opts = AddTorrentOptions::from(meta)
            .output_dir(PathBuf::from(data.resolved_save_path()))
            .paused(data.is_paused())
            .category(data.category.clone())
            .tags(data.tags.clone())
            .sequential(data.is_sequential())
            .file_priorities(FilePriority::decode_list(&data.file_priorities))
            .auto_tmm(data.is_auto_tmm())
            .force_start(data.is_force_start());
        if let Some(limit) = data.ratio_limit_override() {
            opts = opts.ratio_limit(limit);
        }
        if let Some(limit) = data.seeding_time_limit_override() {
            opts = opts.seeding_time_limit(limit);
        }
        let added = self.add_torrent(opts).await?;
        if data.version >= resume::RESUME_VERSION_V4 {
            if let Some(torrent) = self.torrents.get(&added.id.0) {
                torrent
                    .queue_position
                    .store(data.queue_position.max(0) as u64, Ordering::Relaxed);
            }
            if let Some(pending) = self.pending.get(&added.id.0) {
                pending
                    .queue_position
                    .store(data.queue_position.max(0) as u64, Ordering::Relaxed);
            }
            let next = (data.queue_position.max(0) as u64).saturating_add(1);
            let _ = self.next_queue_pos.fetch_max(next, Ordering::Relaxed);
        }
        Ok(())
    }
    ...
    pub fn resume(&self, id: TorrentId) -> Result<(), ControlError> {
        if let Some(torrent) = self.torrents.get(&id.0) {
            ...
            self.set_torrent_state(torrent.value(), id, next);
            self.evaluate_queue();
            if let Some(dht) = self.dht() {
                let enabled = torrent
                    .value()
    ...
    pub fn create_category(
        ...
        }
        {
            ...
            if cats.contains_key(&name) {
                return Err(ControlError::CategoryExists(name));
            }
            cats.insert(name.clone(), Category::new(name, save_path));
        }
    ...
    pub fn edit_category(
    ...
    ) -> Result<(), ControlError> {
        {
            ...
            let category = cats
                .get_mut(name)
                .ok_or_else(|| ControlError::CategoryNotFound(name.to_string()))?;
            category.save_path = save_path.filter(|path| !path.as_os_str().is_empty());
        }
        self.persist_categories()
    }

    pub fn remove_category(&self, name: &str) -> Result<(), ControlError> {
        {
            let mut cats = self.categories.lock().unwrap();
            if cats.remove(name).is_none() {
                return Err(ControlError::CategoryNotFound(name.to_string()));
            }
        }
        for entry in self.torrents.iter() {
            let torrent = entry.value();
            let mut category = torrent.category.lock().unwrap();
        ...
        for entry in self.pending.iter() {
            ...
            let mut category = pending.category.lock().unwrap();
            if category.as_str() == name {
                category.clear();
            }
        }
    ...
    pub fn set_category(
        ...
        if let Some(pending) = self.pending.get(&id.0) {
            if !self.torrents.contains_key(&id.0) {
                *pending.category.lock().unwrap() = category;
                return Ok(());
            }
        }
        let torrent = self
            ...
            .map(|entry| entry.clone())
            .ok_or(ControlError::NotFound(id))?;
        *torrent.category.lock().unwrap() = category;
        self.spawn_flush_one(torrent);
        Ok(())
    }

    pub fn set_tags(&self, id: TorrentId, tags: Vec<String>) -> Result<(), ControlError> {
        self.with_tags(id, |set| {
            set.clear();
            set.extend(normalize_tags(tags));
        })
    }

    pub fn add_tags(&self, id: TorrentId, tags: Vec<String>) -> Result<(), ControlError> {
        self.with_tags(id, |set| {
            set.extend(normalize_tags(tags));
        })
    }

    pub fn remove_tags(&self, id: TorrentId, tags: Vec<String>) -> Result<(), ControlError> {
        self.with_tags(id, |set| {
    ...
    fn with_tags(
        ...
        if let Some(pending) = self.pending.get(&id.0) {
            if !self.torrents.contains_key(&id.0) {
                f(&mut pending.tags.lock().unwrap());
                return Ok(());
            }
        }
        let torrent = self
            ...
            .map(|entry| entry.clone())
            .ok_or(ControlError::NotFound(id))?;
        f(&mut torrent.tags.lock().unwrap());
        self.spawn_flush_one(torrent);
        Ok(())
    }
    ...
    pub async fn set_save_path(
        ...
        if let Some(pending) = self.pending.get(&id.0) {
            if !self.torrents.contains_key(&id.0) {
                *pending.output_dir.lock().unwrap() = path;
                pending.auto_tmm.store(false, Ordering::Relaxed);
                return Ok(());
            }
        }
        let torrent = self
            ...
            .map(|entry| entry.clone())
            .ok_or(ControlError::NotFound(id))?;
        torrent.auto_tmm.store(false, Ordering::Relaxed);
        if !move_files {
            *torrent.output_dir.lock().unwrap() = path;
            self.spawn_flush_one(torrent);
            return Ok(());
        }
        self.relocate_torrent(&torrent, id, path).await
    }
    ...
    async fn relocate_torrent(
        ...
        self.set_torrent_state(torrent, id, TorrentState::Moving);
        match torrent.storage.relocate(&new_output).await {
            Ok(()) => {
                *torrent.output_dir.lock().unwrap() = new_output;
                let next = if previous == TorrentState::Paused
                    || matches!(previous, TorrentState::Error(_))
                {
                    previous
                } else {
            ...
            Err(e) => {
                ...
                self.set_torrent_state(torrent, id, TorrentState::Error(message.clone()));
                self.emit(SessionEvent::Error {
                    id,
                    message: message.clone(),
                });
                Err(ControlError::MoveFailed { id, message })
            }
        }
    }

    async fn maybe_move_on_complete(&self, torrent: &Arc<TorrentSession>, id: TorrentId) {
        if !torrent.auto_tmm.load(Ordering::Relaxed) {
            return;
        }
        let completed_dir = self.options.completed_dir.clone();
        if completed_dir.as_os_str().is_empty() {
            return;
        }
        let category = torrent.category.lock().unwrap().clone();
        let dest_dir = if category.is_empty() {
            completed_dir
        } else {
            completed_dir.join(category)
        };
        let name = torrent.torrent.name.clone();
        let new_output = dest_dir.join(name);
        let current = torrent.output_dir.lock().unwrap().clone();
        if current == new_output {
            return;
        }
        if let Err(e) = self.relocate_torrent(torrent, id, new_output).await {
            warn!(id = %id, error = %e, "failed to move torrent to completed_dir");
        }
    }
    ...
    fn after_content_change(&self, torrent: &Arc<TorrentSession>, id: TorrentId) {
        ...
        if torrent.downloaded_state.is_complete() {
            ...
            if first {
                ...
                tokio::spawn(async move {
                    session.maybe_move_on_complete(&torrent, id).await;
                });
            }
        }
        self.evaluate_queue();
    ...
    pub fn transfer_stats(&self) -> TransferStats {
        ...
        stats.download_limit = self.limits.current_download().rate();
        stats.upload_limit = self.limits.current_upload().rate();
    ...
    pub fn set_rate_limits(&self, download: u64, upload: u64) {
        ...
        if download > 0 {
            self.limits.download.set_burst(DEFAULT_BURST);
        }
        if upload > 0 {
            self.limits.upload.set_burst(DEFAULT_BURST);
        }
    }

    pub fn set_alt_limits(&self, download: u64, upload: u64) {
        self.limits.alt_download.set_rate(download);
        self.limits.alt_upload.set_rate(upload);
        if download > 0 {
            self.limits.alt_download.set_burst(DEFAULT_BURST);
        }
        if upload > 0 {
            self.limits.alt_upload.set_burst(DEFAULT_BURST);
        }
    }

    pub fn set_alt_mode(&self, on: bool) {
        self.limits.set_alt_mode(on);
    }

    pub fn alt_mode(&self) -> bool {
        self.limits.alt_mode()
    }
    ...
    pub fn set_torrent_rate_limits(
        ...
        if let Some(torrent) = self.torrents.get(&id.0) {
            torrent.download_limiter.set_rate(download);
            torrent.upload_limiter.set_rate(upload);
            if download > 0 {
                torrent.download_limiter.set_burst(DEFAULT_BURST);
            }
            if upload > 0 {
                torrent.upload_limiter.set_burst(DEFAULT_BURST);
            }
            return Ok(());
        }
        if let Some(pending) = self.pending.get(&id.0) {
            pending.download_limiter.set_rate(download);
            pending.upload_limiter.set_rate(upload);
            return Ok(());
        }
        Err(ControlError::NotFound(id))
    }

    pub fn set_force_start(&self, id: TorrentId, enabled: bool) -> Result<(), ControlError> {
        if let Some(torrent) = self.torrents.get(&id.0) {
            torrent.force_start.store(enabled, Ordering::Relaxed);
            if enabled {
                let next = active_state(&torrent.downloaded_state);
                self.set_torrent_state(torrent.value(), id, next);
            }
        ...
        if let Some(pending) = self.pending.get(&id.0) {
            pending.force_start.store(enabled, Ordering::Relaxed);
            if enabled {
                self.apply_pending_resume(pending.value(), id);
            }
            self.evaluate_queue();
    ...
    pub fn set_queue_position(&self, id: TorrentId, pos: usize) -> Result<(), ControlError> {
        let mut ids = self.queue_order();
        if !ids.contains(&id) {
            return Err(ControlError::NotFound(id));
        }
        ids.retain(|existing| *existing != id);
        let pos = pos.min(ids.len());
        ids.insert(pos, id);
        self.apply_queue_order(&ids);
        self.evaluate_queue();
        self.spawn_flush_resume();
        Ok(())
    }

    pub fn queue_top(&self, id: TorrentId) -> Result<(), ControlError> {
        self.set_queue_position(id, 0)
    }

    pub fn queue_bottom(&self, id: TorrentId) -> Result<(), ControlError> {
        let len = self.queue_order().len();
        self.set_queue_position(id, len.saturating_sub(1))
    }

    pub fn queue_up(&self, id: TorrentId) -> Result<(), ControlError> {
        let ids = self.queue_order();
        let Some(idx) = ids.iter().position(|existing| *existing == id) else {
            return Err(ControlError::NotFound(id));
        };
        self.set_queue_position(id, idx.saturating_sub(1))
    }

    pub fn queue_down(&self, id: TorrentId) -> Result<(), ControlError> {
        let ids = self.queue_order();
        let Some(idx) = ids.iter().position(|existing| *existing == id) else {
            return Err(ControlError::NotFound(id));
        };
        self.set_queue_position(id, idx.saturating_add(1))
    }
    ...
    pub async fn recheck(&self, id: TorrentId) -> Result<(), ControlError> {
        ...
        self.set_torrent_state(&torrent, id, next);
        if torrent.downloaded_state.is_complete() {
            self.maybe_move_on_complete(&torrent, id).await;
        }
        self.spawn_flush_one(torrent);
        Ok(())
    }
    ...
    fn queue_order(&self) -> Vec<TorrentId> {
        let mut items: Vec<(u64, TorrentId)> = Vec::new();
        for entry in self.torrents.iter() {
            items.push((
                entry.queue_position.load(Ordering::Relaxed),
                TorrentId::new(*entry.key()),
            ));
        }
        for entry in self.pending.iter() {
            if self.torrents.contains_key(entry.key()) {
        ...
        items.sort_by_key(|(pos, id)| (*pos, id.0));
        items.into_iter().map(|(_, id)| id).collect()
    }

    fn apply_queue_order(&self, ids: &[TorrentId]) {
        for (i, id) in ids.iter().enumerate() {
            let pos = i as u64;
            if let Some(torrent) = self.torrents.get(&id.0) {
                torrent.queue_position.store(pos, Ordering::Relaxed);
            }
            if let Some(pending) = self.pending.get(&id.0) {
                pending.queue_position.store(pos, Ordering::Relaxed);
            }
        }
        self.next_queue_pos
    ...
    fn alloc_queue_position(&self) -> u64 {
        self.next_queue_pos.fetch_add(1, Ordering::Relaxed)
    }
    ...
    fn torrent_is_slow(
        ...
        if !self.options.dont_count_slow {
            return false;
        }
        let Some(since) = slow_since else {
            return false;
        };
        let rate = download_rate.max(0.0) + upload_rate.max(0.0);
        rate < SLOW_RATE_BPS as f64 && since.elapsed() >= self.options.queue_slow_window
    }

    fn evaluate_queue(&self) {
        ...
        let mut dl = 0usize;
        let mut ul = 0usize;
        let max_dl = self.options.max_active_downloads;
        let max_ul = self.options.max_active_uploads;
        let max_all = self.options.max_active;

        let mut assignments: Vec<(TorrentId, TorrentState)> = Vec::new();
        for id in order {
            if let Some(torrent) = self.torrents.get(&id.0) {
                let current = torrent.torrent_state.lock().unwrap().clone();
                if matches!(
                    current,
                    TorrentState::Paused
                ...
                if self.seed_limit_reached(&torrent) {
                    assignments.push((id, TorrentState::Paused));
                    continue;
                }
                let complete = torrent.downloaded_state.is_complete();
                let want = if complete {
                    TorrentState::Seeding
                } else {
                    TorrentState::Downloading
                };
                let force = torrent.force_start.load(Ordering::Relaxed);
                let rates = torrent.rates.lock().unwrap();
                let slow = self.torrent_is_slow(
                    rates.download_rate,
                    rates.upload_rate,
                    *torrent.slow_since.lock().unwrap(),
                );
                drop(rates);
                let counts = !slow;
                let ok =
                    force || slot_available(complete, !complete, dl, ul, max_dl, max_ul, max_all);
                if ok {
                    if counts {
                        if complete {
                            ul += 1;
                    ...
                    assignments.push((id, want));
                } else {
                    assignments.push((id, TorrentState::Queued));
                }
            } else if let Some(pending) = self.pending.get(&id.0) {
                let current = pending.torrent_state.lock().unwrap().clone();
                if matches!(
                    current,
                    TorrentState::Paused
                        | TorrentState::Checking
                ...
                let force = pending.force_start.load(Ordering::Relaxed);
                let rates = pending.rates.lock().unwrap();
                let slow = self.torrent_is_slow(
                    rates.download_rate,
                    rates.upload_rate,
                    *pending.slow_since.lock().unwrap(),
                );
                drop(rates);
                let counts = !slow;
                let ok = force || slot_available(false, true, dl, ul, max_dl, max_ul, max_all);
                if ok {
                    if counts {
                        dl += 1;
                    }
                    assignments.push((id, TorrentState::Metadata));
                } else {
                    assignments.push((id, TorrentState::Queued));
                }
            }
        ...
        for (id, state) in assignments {
            if let Some(torrent) = self.torrents.get(&id.0) {
                self.set_torrent_state(torrent.value(), id, state);
                if let Some(dht) = self.dht() {
                    dht.set_active(
                        id.0,
                        torrent.torrent_state.lock().unwrap().transfer_enabled(),
                    );
                }
            } else if let Some(pending) = self.pending.get(&id.0) {
                let changed = {
                ...
                sync_download_state(&pending.download_state, &state);
                if state == TorrentState::Queued {
    ...
    fn seed_limit_reached(&self, torrent: &TorrentSession) -> bool {
        if !torrent.downloaded_state.is_complete() {
            return false;
        }
        ...
        let dl = if downloaded == 0 { size } else { downloaded };
        let ratio = *torrent.ratio_limit.lock().unwrap();
        let ratio_limit = ratio.unwrap_or(self.options.seed_ratio_limit);
        if ratio_limit > 0.0 && dl > 0 && uploaded as f64 / dl as f64 >= ratio_limit {
            return true;
        }
        let time = *torrent.seeding_time_limit.lock().unwrap();
        let minutes = time.unwrap_or(self.options.seed_time_limit);
        if minutes > 0 {
            if let Some(done) = *torrent.completed_at.lock().unwrap() {
                let now = resume::now_unix();
                if now.saturating_sub(done) >= (minutes as i64).saturating_mul(60) {
                    return true;
                }
            }
        }
    ...
    fn update_slow_since(sample: &RateSample, slow_since: &Mutex<Option<std::time::Instant>>) {
        let rate = sample.download_rate.max(0.0) + sample.upload_rate.max(0.0);
        let mut since = slow_since.lock().unwrap();
        if rate < SLOW_RATE_BPS as f64 {
            if since.is_none() {
    ...
    fn load_categories(&self) {
        ...
        let loaded = crate::library::load_categories(state_dir);
        *self.categories.lock().unwrap() = loaded;
    }

    fn persist_categories(&self) -> Result<(), ControlError> {
        let Some(state_dir) = self.options.state_dir.as_ref() else {
            return Ok(());
        };
        let cats = self.categories.lock().unwrap().clone();
        crate::library::save_categories(state_dir, &cats)
            .map_err(|e| ControlError::CategoryPersist(e.to_string()))
    }

    fn resolve_output_dir(&self, name: &str, category: &str) -> PathBuf {
        if !category.is_empty() {
        ...
        let download = &self.options.download_dir;
    ...
    fn ensure_category(&self, name: &str) -> Result<(), ControlError> {
        if name.is_empty() {
            return Ok(());
        }
        {
            ...
            cats.insert(name.to_string(), Category::new(name, None));
        }
        self.persist_categories()
    }

    fn spawn_watch(&self) {
        if self.options.watch_dir.as_os_str().is_empty() {
            return;
        }
        let session = self.share();
        let poll = self.options.watch_poll;
        tokio::spawn(async move {
            let mut seen = std::collections::HashSet::new();
            ...
            loop {
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        session.poll_watch_dir(&mut seen).await;
                    }
                }
            }
        });
    ...
    async fn poll_watch_dir(&self, seen: &mut std::collections::HashSet<PathBuf>) {
        let watch_dir = self.options.watch_dir.clone();
        if watch_dir.as_os_str().is_empty() {
            return;
        }
        for path in crate::library::collect_watch_files(&watch_dir) {
            if !seen.insert(path.clone()) {
                continue;
            }
            if let Err(e) = self.ingest_watch_path(&path).await {
                debug!(path = %path.display(), error = %e, "watch dir skipped file");
            }
        }
    }

    async fn ingest_watch_path(&self, path: &Path) -> anyhow::Result<()> {
        let is_torrent = crate::library::is_watch_torrent(path);
        let is_magnet = crate::library::is_watch_magnet_name(path)
            || crate::library::file_starts_with_magnet(path);
        ...
        let mut opts =
            if is_torrent {
                AddTorrentOptions::from_path(path.to_str().ok_or_else(|| {
                    anyhow::anyhow!("watch path is not utf-8: {}", path.display())
                })?)?
            } else {
                let text = std::fs::read_to_string(path)?;
                let magnet = crate::magnet::Magnet::parse(text.trim())?;
                AddTorrentOptions::from_magnet(&magnet)
            };
        if let Some(category) = crate::library::watch_subdir_category(&self.options.watch_dir, path)
        {
            opts = opts.category(category);
        }

        match self.add_torrent(opts).await {
            Ok(_) => {}
            Err(e) => {
                debug!(path = %path.display(), error = %e, "watch dir add failed");
            }
        }
        self.finish_watch_file(path);
        Ok(())
    }

    fn finish_watch_file(&self, path: &Path) {
        if self.options.watch_delete {
            if let Err(e) = std::fs::remove_file(path) {
                if e.kind() != std::io::ErrorKind::NotFound {
                    debug!(path = %path.display(), error = %e, "failed to delete watched file");
                }
            }
            return;
        }
        ...
        let processed = crate::library::watch_processed_path(state_dir);
        if let Err(e) = std::fs::create_dir_all(&processed) {
            debug!(path = %processed.display(), error = %e, "failed to create watch-processed");
            return;
        }
        let dest = crate::library::unique_processed_path(&processed, path);
        if let Err(e) = std::fs::rename(path, &dest) {
            debug!(
                from = %path.display(),
                to = %dest.display(),
                error = %e,
                "failed to move watched file"
            );
        }
    }
    ...
    fn spawn_alt_schedule(&self) {
        let Some(schedule) = AltSchedule::parse(&self.options.alt_schedule) else {
            return;
        };
        let session = self.share();
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = session.cancel.cancelled() => break,
                    _ = interval.tick() => {
                        session.set_alt_mode(schedule.active_now());
                    }
                }
            }
        });
    ...
    fn tick(&self) {
        tick_session(&self.torrents, &self.pending, &self.event_tx);
        for entry in self.torrents.iter() {
            let torrent = entry.value();
            let rates = torrent.rates.lock().unwrap();
            Self::update_slow_since(&rates, &torrent.slow_since);
        }
        for entry in self.pending.iter() {
            if self.torrents.contains_key(entry.key()) {
                continue;
            ...
            let pending = entry.value();
            let rates = pending.rates.lock().unwrap();
            Self::update_slow_since(&rates, &pending.slow_since);
        }
        let mut pause_ids = Vec::new();
        for entry in self.torrents.iter() {
            let id = TorrentId::new(*entry.key());
            let state = entry.torrent_state.lock().unwrap().clone();
            if matches!(state, TorrentState::Seeding | TorrentState::Downloading)
                && self.seed_limit_reached(entry.value())
            {
                pause_ids.push(id);
            }
        }
    ...
    pub fn connect_peer(&self, info_hash: &[u8; 20], addr: SocketAddr) -> bool {
        ...
        if let Some(torrent) = self.torrents.get(info_hash).map(|entry| entry.clone()) {
            if *torrent.torrent_state.lock().unwrap() == TorrentState::Queued {
                return false;
            }
        ...
        if *pending.torrent_state.lock().unwrap() == TorrentState::Queued {
            return false;
        }
    ...
    pub fn remove_torrent(&self, id: TorrentId, delete_files: bool) -> Result<(), ControlError> {
        ...
        if let Some(torrent) = torrent {
            ...
            if delete_files {
                let output_dir = torrent.output_dir.lock().unwrap().clone();
                delete_torrent_files(&torrent.torrent, &output_dir);
            }
        } else if let Some(pending) = pending {
            disconnect_all_peers(&pending.peer_states);
            if delete_files {
                let output_dir = pending.output_dir.lock().unwrap().clone();
                delete_torrent_files(&pending.torrent.get(), &output_dir);
            }
        }
    ...
    async fn register_pending(
        ...
        let limits = BandwidthLimiters::new(
            self.limits.clone(),
            download_limiter.clone(),
            upload_limiter.clone(),
        );
        let queue_position = self.alloc_queue_position();
        let pending = Arc::new(PendingTorrent {
            ...
            category: Mutex::new(opts.category.clone()),
            tags: Mutex::new(normalize_tags(opts.tags.clone())),
            auto_tmm: AtomicBool::new(auto_tmm),
            ...
            queue_position: AtomicU64::new(queue_position),
            force_start: AtomicBool::new(opts.force_start),
            download_limiter,
            upload_limiter,
            limits,
    ...
    async fn add_torrent_meta(
        ...
        let category = if add_torrent.category.is_empty() {
            reuse
                .as_ref()
                .map(|p| p.category.lock().unwrap().clone())
                .unwrap_or_default()
        } else {
            add_torrent.category.clone()
        };
        if !category.is_empty() {
            let _ = self.ensure_category(&category);
        }
        let tags = if add_torrent.tags.is_empty() {
            reuse
                .as_ref()
                .map(|p| p.tags.lock().unwrap().clone())
                .unwrap_or_default()
        } else {
            normalize_tags(add_torrent.tags.clone())
        };
        let explicit_save = add_torrent.output_dir.is_some();
        let output_dir = add_torrent
            .output_dir
            .clone()
            .or_else(|| reuse.as_ref().map(|p| p.output_dir.lock().unwrap().clone()))
            .unwrap_or_else(|| self.resolve_output_dir(&torrent.name, &category));
        let auto_tmm = add_torrent.auto_tmm.unwrap_or_else(|| {
            reuse
                .as_ref()
                .map(|p| p.auto_tmm.load(Ordering::Relaxed))
                .unwrap_or(!explicit_save)
        });
        ...
        let initial_state = if start_paused {
            TorrentState::Paused
        } else if self.restoring.load(Ordering::Relaxed) {
            TorrentState::Queued
        } else {
        ...
        *torrent_state.lock().unwrap() = initial_state.clone();
        ...
        let download_limiter = reuse
            .as_ref()
            .map(|p| p.download_limiter.clone())
            .unwrap_or_else(|| {
                Arc::new(RateLimiter::new(add_torrent.download_limit, DEFAULT_BURST))
            });
        let upload_limiter = reuse
            .as_ref()
            .map(|p| p.upload_limiter.clone())
            .unwrap_or_else(|| Arc::new(RateLimiter::new(add_torrent.upload_limit, DEFAULT_BURST)));
        if reuse.is_none() {
            download_limiter.set_rate(add_torrent.download_limit);
            upload_limiter.set_rate(add_torrent.upload_limit);
        }
        let limits = BandwidthLimiters::new(
            self.limits.clone(),
            download_limiter.clone(),
        ...
        let queue_position = reuse
            .as_ref()
            .map(|p| p.queue_position.load(Ordering::Relaxed))
            .unwrap_or_else(|| self.alloc_queue_position());
        let force_start = add_torrent.force_start
            || reuse
                .as_ref()
                .is_some_and(|p| p.force_start.load(Ordering::Relaxed));
        let ratio_limit = add_torrent
            .ratio_limit
            .or_else(|| reuse.as_ref().and_then(|p| *p.ratio_limit.lock().unwrap()));
        let seeding_time_limit = add_torrent.seeding_time_limit.or_else(|| {
            reuse
                .as_ref()
                .and_then(|p| *p.seeding_time_limit.lock().unwrap())
        });
        ...
        let torrent_session = Arc::new(TorrentSession {
            ...
            rates,
            queue_position: AtomicU64::new(queue_position),
            force_start: AtomicBool::new(force_start),
            download_limiter,
            upload_limiter,
            limits,
            ratio_limit: Mutex::new(ratio_limit),
            seeding_time_limit: Mutex::new(seeding_time_limit),
            slow_since: Mutex::new(None),
        });
        ...
        let writer_session = self.share();
        let writer_torrent = torrent_session.clone();
        tokio::spawn(async move {
            loop {
                ...
                if let Err(e) = writer_torrent
                    .storage
                    .write_piece(piece.index, &piece.buf)
                    .await
                {
                    debug!(index = piece.index, error = %e, "failed to write piece");
                    writer_torrent
                        .downloaded_state
                        .remove_downloaded(piece.index);
                    continue;
                }
                if writer_torrent.downloaded_state.is_complete() {
                    let first_complete = {
                        let mut done = writer_torrent.completed_at.lock().unwrap();
                        if done.is_none() {
                            *done = Some(resume::now_unix());
                            true
                        } else {
                    ...
                    if first_complete {
                        if let Err(e) = writer_torrent.storage.sync_all().await {
                            debug!(error = %e, "failed to sync files on completion");
                        }
                        writer_session.emit(SessionEvent::Completed { id });
                        let was_downloading = {
                            let state = writer_torrent.torrent_state.lock().unwrap().clone();
                            state == TorrentState::Downloading
                        };
                        if was_downloading {
                            writer_session.set_torrent_state(
                                &writer_torrent,
                                id,
                                TorrentState::Seeding,
                            );
                        }
                        writer_session.evaluate_queue();
                        writer_session
                            .maybe_move_on_complete(&writer_torrent, id)
                            .await;
                    }
                }
                if let Some(state_dir) = writer_session.options.state_dir.as_ref() {
                    if let Err(e) = persist_torrent(state_dir, &writer_torrent) {
        ...
        if let Some(state_dir) = self.options.state_dir.clone() {
            ...
            spawn_resume_timer(torrent_session.clone(), state_dir, self.cancel.clone());
        }
        ...
        if torrent_session.downloaded_state.is_complete() {
            let session = self.share();
            let torrent = torrent_session.clone();
            tokio::spawn(async move {
                session.maybe_move_on_complete(&torrent, id).await;
            });
        }
    ...
    async fn add_magnet(
        ...
        if !add_torrent.category.is_empty() {
            let _ = self.ensure_category(&add_torrent.category);
        }
        let output_dir = add_torrent.output_dir.clone().unwrap_or_else(|| {
            self.resolve_output_dir(&magnet.name_or_hash(), &add_torrent.category)
        });
        let pending = self
            .register_pending(
                magnet.info_hash,
        ...
        tokio::spawn(async move {
            ...
            opts.output_dir = Some(pending_task.output_dir.lock().unwrap().clone());
            opts.verify = pending_task.verify;
            opts.skip_checking = pending_task.skip_checking;
            opts.paused = Some(pending_task.start_paused);
            opts.category = pending_task.category.lock().unwrap().clone();
            opts.tags = pending_task.tags.lock().unwrap().iter().cloned().collect();
            ...
            opts.auto_tmm = Some(pending_task.auto_tmm.load(Ordering::Relaxed));
            opts.force_start = pending_task.force_start.load(Ordering::Relaxed);
            opts.ratio_limit = *pending_task.ratio_limit.lock().unwrap();
            opts.seeding_time_limit = *pending_task.seeding_time_limit.lock().unwrap();
...
fn persist_torrent(
    ...
    let output_dir = torrent.output_dir.lock().unwrap().clone();
    let category = torrent.category.lock().unwrap().clone();
    let tags: Vec<String> = torrent.tags.lock().unwrap().iter().cloned().collect();
    persist_from_parts(
        state_dir,
        ...
        *torrent.completed_at.lock().unwrap(),
        ...
        &output_dir,
        torrent.auto_tmm.load(Ordering::Relaxed),
        torrent.queue_position.load(Ordering::Relaxed) as i64,
        torrent.force_start.load(Ordering::Relaxed),
        encode_ratio_milli(*torrent.ratio_limit.lock().unwrap()),
        encode_time_limit(*torrent.seeding_time_limit.lock().unwrap()),
    )
}

fn encode_ratio_milli(limit: Option<f64>) -> i64 {
    match limit {
        None => 0,
        Some(v) if v <= 0.0 => -1,
...
fn encode_time_limit(limit: Option<u64>) -> i64 {
    match limit {
        None => 0,
        Some(0) => -1,
        Some(v) => v as i64,
...
fn slot_available(
    ...
    if max_all > 0 && dl + ul >= max_all {
        return false;
    }
    if downloading && max_dl > 0 && dl >= max_dl {
        return false;
    }
    if seeding && max_ul > 0 && ul >= max_ul {
        return false;
    }
    true
}
...
fn torrent_snapshot(torrent: &TorrentSession) -> TorrentSnapshot {
    ...
    TorrentSnapshot {
        ...
        peers,
        seeds,
        save_path: torrent.output_dir.lock().unwrap().clone(),
        torrent_path: torrent.torrent_cache_path.clone(),
        category: torrent.category.lock().unwrap().clone(),
        tags: torrent.tags.lock().unwrap().iter().cloned().collect(),
...
fn pending_snapshot(info_hash: [u8; 20], pending: &PendingTorrent) -> TorrentSnapshot {
    ...
    TorrentSnapshot {
        ...
        peers,
        seeds,
        save_path: pending.output_dir.lock().unwrap().clone(),
        torrent_path: PathBuf::new(),
        category: pending.category.lock().unwrap().clone(),
        tags: pending.tags.lock().unwrap().iter().cloned().collect(),
...
pub async fn accept_incoming_stream(incoming: IncomingStream, ctx: IncomingPeerContext) {
    ...
    if *target.torrent_state.lock().unwrap() == TorrentState::Queued {
        debug!(%addr, "refusing incoming peer for queued torrent");
        return;
    }
...
impl IncomingTarget {
    fn from_session(entry: &TorrentSession) -> Self {
        Self {
            ...
            promote_notify: Arc::new(Notify::new()),
            torrent_state: entry.torrent_state.clone(),
